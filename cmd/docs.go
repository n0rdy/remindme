...
var docsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		logger.Info("docs command: called")
...
func getDir(cmd *cobra.Command) (string, error) {
	...
	if err != nil {
		logger.Error("docs command: error while parsing flag: "+common.DirFlag, err)
		return "", common.ErrWrongFormattedStringFlag(common.DirFlag)
	}

	if dir == "" {
		if dir, err = os.MkdirTemp("", "remindme"); err != nil {
			logger.Error("docs command: error while creating temp dir", err)
...
func generateDocs(out io.Writer, dir string) error {
	if err := doc.GenMarkdownTree(rootCmd, dir); err != nil {
		logger.Error("docs command: error while generating docs", err)
		return common.ErrDocsCmdOnDocsGeneration
	}
	_, err := fmt.Fprintf(out, "Documentation successfully created in %s\n", dir)
	if err != nil {
		logger.Error("docs command: error while writing to output", err)
	}
	return nil
}
